...
const SOLANA_RPC = (process.env.NEXT_PUBLIC_SOLANA_RPC ?? "https://api.devnet.solana.com").trim();
...
// ---------------------------------------------------------------------------
// Step 1: Generate agent identity keypair + derive DID and wallet address
//
// Virtuals equivalent: agent gets a unique address derived from factory + nonce
// Our equivalent: Generate Ed25519 keypair → derive DID + Solana wallet from same seed
// This creates a deterministic link: identity key ↔ DID ↔ wallet address
// ---------------------------------------------------------------------------
...
export function generateAgentIdentity() {
  ...
  // DID derived from pubkey (matches identity.ts generateDID format)
  const hash = createHash("sha256").update(publicKeyHex).digest("hex").slice(0, 32);
  const did = `did:relay:${hash}`;

  // Solana wallet from same seed (Ed25519 → Ed25519, same keypair)
  const solanaKeypair = Keypair.fromSeed(new Uint8Array(seed));
  const walletAddress = solanaKeypair.publicKey.toString();

  return { did, walletAddress, publicKeyHex, privateKeyHex };
}
...
async function insertAgentRecord(supabase, params, did, creatorWallet, agentWalletAddress, agentPublicKeyHex) {
...
export async function createAgent(params, creatorWallet, payerKeypair) {
  ...
  try {
    // ── Step 1: Generate agent identity (keypair → DID + wallet) ────────
    const { did, walletAddress, publicKeyHex } = generateAgentIdentity();
    steps.push({ step: "identity", status: "done", did, walletAddress });

    // ── Step 2: Insert agent in Supabase (status: pending) ──────────────
    const insertResult = await insertAgentRecord(supabase, params, did, creatorWallet, walletAddress, publicKeyHex);
...
export function loadPayerKeypair() {
  const secret = process.env.RELAY_PAYER_SECRET_KEY?.trim();
