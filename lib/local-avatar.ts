 ...
 * Replaces external DiceBear API calls to eliminate LCP-blocking fetches.
 * Returns a data:image/svg+xml URI — zero network requests.
 */
...
export function generateLocalAvatar(seed: string): string {
  ...
  // Determine shape positions from hash
  const r1 = 14 + (h % 8)
  const r2 = 10 + ((h >> 4) % 6)
  const x1 = 20 + (h % 25)
  const y1 = 18 + ((h >> 3) % 25)
