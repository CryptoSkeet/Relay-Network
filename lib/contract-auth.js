...
export async function verifyContractCaller(request) {
  ...
  // ── Path 0: Service token (CRON_SECRET) ───────────────────────────────────
  // The heartbeat daemon needs to act on behalf of arbitrary agents
  // (initiating/settling contracts during autonomous loops). We trust the
  // caller because they hold the CRON_SECRET — same trust model as
  // /api/v1/relay-token/mint and /api/cron/*. The acted-on agent must be
  // declared in the `x-relay-agent-id` header so we can scope the
  // identity. This replaces the heartbeat's old direct-DB-write path
  // (which bypassed on-chain escrow entirely).
  if (
    process.env.CRON_SECRET &&
    authHeader === `Bearer ${process.env.CRON_SECRET}`
  ) {
    const agentId = request.headers.get("x-relay-agent-id");
    if (!agentId) {
      return {
        ok: false,
        error: "Service token requires x-relay-agent-id header",
        status: 400,
      };
    }
    ...
    if (agent.status !== "active") {
      return { ok: false, error: "Agent not active", status: 403 };
    }
    return {
      ok: true,
      identity: {
...
async function verifyApiKey(apiKey) {
  ...
  const { data: keyRecord, error } = await db
    .from("agent_api_keys")
    .select("agent_id, is_active, expires_at, agents(id, status, creator_wallet)")
    .eq("key_hash", keyHash)
    .single();
  ...
  return {
    ...
    identity: {
      ...
      wallet:  keyRecord.agents?.creator_wallet ?? null,
      method:  "api-key",
    },
  };
}
