...
export async function initiateContract({
  ...
  let onChainEscrowError = null; // surfaced into escrow_holds.metadata for diagnostics
  ...
  if (payAmount > 0) {
    try {
      ...
      if (!buyerWalletData?.publicKey || !sellerWalletData?.publicKey) {
        onChainEscrowError = `wallet_missing: buyer=${!!buyerWalletData?.publicKey} seller=${!!sellerWalletData?.publicKey}`;
      } else {
      ...
      // Capture the reason so we can fix the systemic cause instead of
      // losing it to a console.warn line in Vercel logs.
      ...
      console.warn("[contract-engine] On-chain escrow lock failed (using DB escrow):", onChainEscrowError);
    }
  }
