...
export async function settleContract({ contractId, buyerAgentId }) {
  ...
  // 2. On-chain RELAY transfer: buyer → seller
  let onChainSig = null;
  const payAmount = contract.price_relay ?? 0;
  if (payAmount > 0) {
    try {
      // Look up both wallets
      const { data: buyerWallet } = await db
        .from("solana_wallets")
        .select("public_key, encrypted_private_key, encryption_iv")
        .eq("agent_id", contract.buyer_agent_id)
        .maybeSingle();
      ...
      if (buyerWallet?.encrypted_private_key && buyerWallet?.encryption_iv && sellerWallet?.public_key) {
        const { transferRelayOnChain } = await import("@/lib/solana/relay-token");
        onChainSig = await transferRelayOnChain(
          buyerWallet.encrypted_private_key,
          buyerWallet.encryption_iv,
          buyerWallet.public_key,
          sellerWallet.public_key,
          payAmount,
        );
        console.log(`[contract-engine] On-chain settlement tx: ${onChainSig}`);
      } else {
        console.warn("[contract-engine] Missing wallets for on-chain settlement, skipping");
      }
    } catch (txErr) {
      // Non-fatal: log but continue with DB settlement
      console.error("[contract-engine] On-chain transfer failed:", txErr.message || txErr);
    }
  }

  // 3. Credit seller's agent_rewards
    ...
    await db.from("transactions").insert([
      {
        ...
        status:        "completed",
        tx_hash:       onChainSig,
        description:   `Contract settlement: ${contract.title || contractId}`,
      },
    ]).then(() => {});
  }
