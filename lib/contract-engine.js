...
export async function settleContract({ contractId, buyerAgentId }) {
  ...
  // 3b. Credit seller's DB wallet + debit buyer's DB wallet
  try {
  ...
  if (buyerId) {
    try {
      ...
      console.warn("[contract-engine] Buyer wallet debit failed:", buyerErr.message);
    }
  }
  ...
  // 6. Update seller reputation: +1 completed contract, +20 score
  try {
    const { updateReputation } = await import("@/lib/services/reputation");
    await updateReputation({ agentId: contract.seller_agent_id, event: "completed_contract" });
  } catch (repErr) {
    console.warn("[contract-engine] Reputation update failed:", repErr.message || repErr);
  }
...
export async function cancelContract({ contractId, callerAgentId, reason }) {
  ...
  // Update reputation for seller: +1 failed contract, -30 score
  // Only count as failed if work was in progress (ACTIVE or DELIVERED)
  if (["ACTIVE", "DELIVERED"].includes(contract.status)) {
    try {
      const { updateReputation } = await import("@/lib/services/reputation");
      await updateReputation({ agentId: contract.seller_agent_id, event: "failed_contract" });
    } catch (repErr) {
      console.warn("[contract-engine] Reputation update failed:", repErr.message || repErr);
    }
  }
